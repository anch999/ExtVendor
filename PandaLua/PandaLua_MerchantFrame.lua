...
function PandaLua_MerchantFrame_OnLoad(self)
	...
	UIDropDownMenu_Initialize(self.lootFilter, PandaLua_MerchantFrame_InitFilter);
end

function PandaLua_MerchantFrame_OnEvent(self, event, ...)
	if ( event == "MERCHANT_UPDATE" and "MERCHANT_FILTER_ITEM_UPDATE" ) then
		self.update = true;
	elseif ( event == "MERCHANT_CLOSED" ) then
		self:UnregisterEvent("CURRENCY_DISPLAY_UPDATE");
		HidePandaLua_UIPanel(self);
	elseif ( event == "MERCHANT_SHOW" ) then
		ShowPandaLua_UIPanel(self);
		if ( not self:IsShown() ) then
			CloseMerchant();
			return;
		end
		self.page = 1;
		PandaLua_MerchantFrame_UpdateCurrencies();
		PandaLua_MerchantFrame_Update();
	elseif ( event == "PLAYER_MONEY" or event == "GUILDBANK_UPDATE_MONEY" or event == "GUILDBANK_UPDATE_WITHDRAWMONEY" ) then
		PandaLua_MerchantFrame_UpdateCanRepairAll();
		PandaLua_MerchantFrame_UpdateRepairButtons();
	elseif ( event == "CURRENCY_DISPLAY_UPDATE" ) then
		PandaLua_MerchantFrame_UpdateCurrencyAmounts();
	end
end

function PandaLua_MerchantFrame_OnShow(self)
	OpenAllBags(self);
	
	-- Update repair all button status
	PandaLua_MerchantFrame_UpdateCanRepairAll();
	PandaLua_MerchantFrame_UpdateGuildBankRepair();
	PanelTemplates_SetTab(PandaLua_MerchantFrame, 1);
...
function PandaLua_MerchantFrame_OnHide(self)
	CloseMerchant();
	CloseAllBags(self);
	ResetCursor();
...
function PandaLua_MerchantFrame_Update()
	PandaLua_MerchantFrame_UpdateFilterString()
	if ( PandaLua_MerchantFrame.selectedTab == 1 ) then
		PandaLua_MerchantFrame_UpdateMerchantInfo();
	else
		PandaLua_MerchantFrame_UpdateBuybackInfo();
	end
	
end

function PandaLua_MerchantFrame_UpdateMerchantInfo()
	MerchantNameText:SetText(UnitName("NPC"));
	SetPortraitTexture(PandaLua_MerchantFramePortrait, "NPC");
	
	local numMerchantItems = GetMerchantNumItems();
	
	MerchantPageText:SetFormattedText(MERCHANT_PAGE_NUMBER, PandaLua_MerchantFrame.page, math.ceil(numMerchantItems / MERCHANT_ITEMS_PER_PAGE));

	local name, texture, price, stackCount, numAvailable, isUsable, extendedCost;
	for i=1, MERCHANT_ITEMS_PER_PAGE, 1 do
		local index = (((PandaLua_MerchantFrame.page - 1) * MERCHANT_ITEMS_PER_PAGE) + i);
		...
		if ( index <= numMerchantItems ) then
			...
			if ( extendedCost and (price <= 0) ) then
				...
				PandaLua_MerchantFrame_UpdateAltCurrency(index, i);
	...
	if ( numMerchantItems > MERCHANT_ITEMS_PER_PAGE ) then
		if ( PandaLua_MerchantFrame.page == 1 ) then
			MerchantPrevPageButton:Disable();
		else
			MerchantPrevPageButton:Enable();
		end
		if ( PandaLua_MerchantFrame.page == ceil(numMerchantItems / MERCHANT_ITEMS_PER_PAGE) or numMerchantItems == 0) then
	...
	MerchantBuyBackItem:Show();
	PandaLua_MerchantFrameBottomLeftBorder:Show();
	PandaLua_MerchantFrameBottomRightBorder:Show();
...
function PandaLua_MerchantFrame_UpdateAltCurrency(index, i)
...
function PandaLua_MerchantFrame_UpdateBuybackInfo()
	MerchantNameText:SetText(MERCHANT_BUYBACK);
	PandaLua_MerchantFramePortrait:SetTexture("PandaLua\InterfaceElements\\PandaLua_MerchantFrame\\UI-BuyBack-Icon");
	...
	MerchantPrevPageButton:Hide();
	MerchantNextPageButton:Hide();
	PandaLua_MerchantFrameBottomLeftBorder:Hide();
	PandaLua_MerchantFrameBottomRightBorder:Hide();
...
function MerchantPrevPageButton_OnClick()
	...
	PandaLua_MerchantFrame.page = PandaLua_MerchantFrame.page - 1;
	PandaLua_MerchantFrame_Update();
end
...
function MerchantItemButton_OnLoad(self)
	...
	self.SplitStack = function(button, split)
		if ( button.extendedCost ) then
			PandaLua_MerchantFrame_ConfirmExtendedItemCost(button, split)
		elseif ( split > 0 ) then
			BuyMerchantItem(button:GetID(), split);
		end
	end
...
function MerchantItemButton_OnClick(self, button)
	PandaLua_MerchantFrame.extendedCost = nil;
	PandaLua_MerchantFrame.highPrice = nil;
	
	if ( PandaLua_MerchantFrame.selectedTab == 1 ) then
		-- Is merchant frame
		if ( button == "LeftButton" ) then
			if ( PandaLua_MerchantFrame.refundItem ) then
				if ( ContainerFrame_GetExtendedPriceString(PandaLua_MerchantFrame.refundItem, PandaLua_MerchantFrame.refundItemEquipped)) then
			...
			if ( self.extendedCost ) then
				PandaLua_MerchantFrame.extendedCost = self;
			elseif ( self.price and self.price >= MERCHANT_HIGH_PRICE_COST ) then
				PandaLua_MerchantFrame.highPrice = self;
			end
		else
			if ( self.extendedCost ) then
				PandaLua_MerchantFrame_ConfirmExtendedItemCost(self);
			elseif ( self.price and self.price >= MERCHANT_HIGH_PRICE_COST ) then
				PandaLua_MerchantFrame_ConfirmHighCostItem(self);
			else
...
function MerchantItemButton_OnEnter(button)
	...
	if ( PandaLua_MerchantFrame.selectedTab == 1 ) then
		...
		PandaLua_MerchantFrame.itemHover = button:GetID();
	else
...
function PandaLua_MerchantFrame_ConfirmExtendedItemCost(itemButton, numToPurchase)
	...
	PandaLua_MerchantFrame.itemIndex = index;
	PandaLua_MerchantFrame.count = numToPurchase;
...
function PandaLua_MerchantFrame_ResetRefundItem()
	PandaLua_MerchantFrame.refundItem = nil;
	PandaLua_MerchantFrame.refundItemEquipped = nil;
end

function PandaLua_MerchantFrame_SetRefundItem(item, isEquipped)
	PandaLua_MerchantFrame.refundItem = item;
	PandaLua_MerchantFrame.refundItemEquipped = isEquipped;
end

function PandaLua_MerchantFrame_ConfirmHighCostItem(itemButton, quantity)
	quantity = (quantity or 1);
	local index = itemButton:GetID();

	PandaLua_MerchantFrame.itemIndex = index;
	PandaLua_MerchantFrame.count = quantity;
	PandaLua_MerchantFrame.price = itemButton.price;
	StaticPopup_Show("CONFIRM_HIGH_COST_ITEM", itemButton.link);
end

function PandaLua_MerchantFrame_UpdateCanRepairAll()
...
function PandaLua_MerchantFrame_UpdateGuildBankRepair()
	local repairAllCost, canRepair = GetRepairAllCost();
	if ( canRepair ) then
...
function PandaLua_MerchantFrame_UpdateRepairButtons()
	if ( CanMerchantRepair() ) then
		...
		if ( CanGuildBankRepair() ) then
			...
			MerchantRepairAllButton:SetPoint("BOTTOMRIGHT", PandaLua_MerchantFrame, "BOTTOMLEFT", 100, 30);
			MerchantRepairText:ClearAllPoints();
			MerchantRepairText:SetPoint("CENTER", PandaLua_MerchantFrame, "BOTTOMLEFT", 80, 68);
			...
			MerchantRepairAllButton:SetPoint("BOTTOMRIGHT", PandaLua_MerchantFrame, "BOTTOMLEFT", 160, 32);
			MerchantRepairText:ClearAllPoints();
			MerchantRepairText:SetPoint("BOTTOMLEFT", PandaLua_MerchantFrame, "BOTTOMLEFT", 14, 45);
			MerchantGuildBankRepairButton:Hide();
		end
...
function PandaLua_MerchantFrame_UpdateCurrencies()
	local currencies = { GetMerchantCurrencies() };
	
	if ( #currencies == 0 ) then	-- common case
		PandaLua_MerchantFrame:UnregisterEvent("CURRENCY_DISPLAY_UPDATE");
		...
		PandaLua_MerchantFrame:RegisterEvent("CURRENCY_DISPLAY_UPDATE");
		MerchantExtraCurrencyInset:Show();
		MerchantExtraCurrencyBg:Show();
		PandaLua_MerchantFrame_OrderCurrencies(currencies);
		...
		for index = 1, numCurrencies do
			...
			if ( not tokenButton ) then
				tokenButton = CreateFrame("BUTTON", "MerchantToken"..index, PandaLua_MerchantFrame, "BackpackTokenTemplate");
				...
				tokenButton:SetScript("OnEnter", PandaLua_MerchantFrame_ShowCurrencyTooltip);
			end
...
function PandaLua_MerchantFrame_OrderCurrencies(currencyTable)
...
function PandaLua_MerchantFrame_ShowCurrencyTooltip(self)
	GameTooltip:SetOwner(self, "ANCHOR_RIGHT");
	GameTooltip:SetCurrencyByID(self.currencyID);
end

function PandaLua_MerchantFrame_UpdateCurrencyAmounts()
...
function PandaLua_MerchantFrame_SetFilter(self, classIndex)
	SetMerchantFilter(classIndex);
	PandaLua_MerchantFrame.page = 1;
	if PandaLua_MerchantFrame:IsVisible() then
		PandaLua_MerchantFrame_Update();
	end
end

function PandaLua_MerchantFrame_UpdateFilterString()
	local name = ALL;
	...
	UIDropDownMenu_SetText(PandaLua_MerchantFrame.lootFilter, name);
end

function PandaLua_MerchantFrame_InitFilter()
	local info = UIDropDownMenu_CreateInfo();
	local currFilter = GetMerchantFilter();
	local className = UnitClass("player");

	info.func = PandaLua_MerchantFrame_SetFilter;
