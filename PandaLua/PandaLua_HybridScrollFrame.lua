--[[-----------------------------------------------------------------------------------------------
	For a PandaLua_Hybrid scroll frame with buttons of varying size, set .dynamic on the scroll frame
...
function PandaLua_HybridScrollFrame_OnLoad (self)
	self:EnableMouse(true);
end

function PandaLua_HybridScrollFrameScrollUp_OnLoad (self)
...
function PandaLua_HybridScrollFrameScrollDown_OnLoad (self)
...
function PandaLua_HybridScrollFrame_OnValueChanged (self, value)
	PandaLua_HybridScrollFrame_SetOffset(self:GetParent(), value);
	PandaLua_HybridScrollFrame_UpdateButtonStates(self:GetParent(), value);
end
	
function PandaLua_HybridScrollFrame_UpdateButtonStates (self, currValue)
...
function PandaLua_HybridScrollFrame_OnMouseWheel (self, delta, stepSize)
...
function PandaLua_HybridScrollFrameScrollButton_OnUpdate (self, elapsed)
	...
	if ( self.timeSinceLast >= ( self.updateInterval or 0.08 ) ) then
		...
		elseif ( self:IsMouseOver() ) then
			...
			PandaLua_HybridScrollFrame_OnMouseWheel (parent, self.direction, (self.stepSize or parent.buttonHeight/3));
			self.timeSinceLast = 0;
		end
	end
end

function PandaLua_HybridScrollFrameScrollButton_OnClick (self, button, down)
	local parent = self.parent or self:GetParent():GetParent();
	
	if ( down ) then
		self.timeSinceLast = (self.timeToStart or -0.2);
		self:SetScript("OnUpdate", PandaLua_HybridScrollFrameScrollButton_OnUpdate);
		PandaLua_HybridScrollFrame_OnMouseWheel (parent, self.direction);
...
function PandaLua_HybridScrollFrame_Update (self, totalHeight, displayedHeight)
	...
	if ( range > 0 and self.scrollBar ) then
		...
		if ( math.floor(self.scrollBar:GetValue()) >= math.floor(maxVal) ) then
			...
			if ( math.floor(self.scrollBar:GetValue()) ~= math.floor(range) ) then
				...
				PandaLua_HybridScrollFrame_SetOffset(self, range); -- If we've scrolled to the bottom, we need to recalculate the offset.
			end
		else
			self.scrollBar:SetMinMaxValues(0, range)
		end
		self.scrollBar:Enable();
		PandaLua_HybridScrollFrame_UpdateButtonStates(self);
		self.scrollBar:Show();
...
function PandaLua_HybridScrollFrame_GetOffset (self)
	return math.floor(self.offset or 0), (self.offset or 0);
end

function PandaLua_HybridScrollFrameScrollChild_OnLoad (self)
	self:GetParent().scrollChild = self;
end

function PandaLua_HybridScrollFrame_ExpandButton (self, offset, height)
	self.largeButtonTop = round(offset);
	self.largeButtonHeight = round(height)
	PandaLua_HybridScrollFrame_SetOffset(self, self.scrollBar:GetValue());
end

function PandaLua_HybridScrollFrame_CollapseButton (self)
	self.largeButtonTop = nil;
	self.largeButtonHeight = nil;
end

function PandaLua_HybridScrollFrame_SetOffset (self, offset)
...
function PandaLua_HybridScrollFrame_CreateButtons (self, buttonTemplate, initialOffsetX, initialOffsetY, initialPoint, initialRelative, offsetX, offsetY, point, relativePoint)
