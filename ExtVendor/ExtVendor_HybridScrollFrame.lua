--[[-----------------------------------------------------------------------------------------------
	For a ExtVendor_Hybrid scroll frame with buttons of varying size, set .dynamic on the scroll frame
...
function ExtVendor_HybridScrollFrame_OnLoad (self)
	self:EnableMouse(true);
end

function ExtVendor_HybridScrollFrameScrollUp_OnLoad (self)
...
function ExtVendor_HybridScrollFrameScrollDown_OnLoad (self)
...
function ExtVendor_HybridScrollFrame_OnValueChanged (self, value)
	ExtVendor_HybridScrollFrame_SetOffset(self:GetParent(), value);
	ExtVendor_HybridScrollFrame_UpdateButtonStates(self:GetParent(), value);
end
	
function ExtVendor_HybridScrollFrame_UpdateButtonStates (self, currValue)
...
function ExtVendor_HybridScrollFrame_OnMouseWheel (self, delta, stepSize)
...
function ExtVendor_HybridScrollFrameScrollButton_OnUpdate (self, elapsed)
	...
	if ( self.timeSinceLast >= ( self.updateInterval or 0.08 ) ) then
		...
		elseif ( self:IsMouseOver() ) then
			...
			ExtVendor_HybridScrollFrame_OnMouseWheel (parent, self.direction, (self.stepSize or parent.buttonHeight/3));
			self.timeSinceLast = 0;
		end
	end
end

function ExtVendor_HybridScrollFrameScrollButton_OnClick (self, button, down)
	local parent = self.parent or self:GetParent():GetParent();
	
	if ( down ) then
		self.timeSinceLast = (self.timeToStart or -0.2);
		self:SetScript("OnUpdate", ExtVendor_HybridScrollFrameScrollButton_OnUpdate);
		ExtVendor_HybridScrollFrame_OnMouseWheel (parent, self.direction);
...
function ExtVendor_HybridScrollFrame_Update (self, totalHeight, displayedHeight)
	...
	if ( range > 0 and self.scrollBar ) then
		...
		if ( math.floor(self.scrollBar:GetValue()) >= math.floor(maxVal) ) then
			...
			if ( math.floor(self.scrollBar:GetValue()) ~= math.floor(range) ) then
				...
				ExtVendor_HybridScrollFrame_SetOffset(self, range); -- If we've scrolled to the bottom, we need to recalculate the offset.
			end
		else
			self.scrollBar:SetMinMaxValues(0, range)
		end
		self.scrollBar:Enable();
		ExtVendor_HybridScrollFrame_UpdateButtonStates(self);
		self.scrollBar:Show();
...
function ExtVendor_HybridScrollFrame_GetOffset (self)
	return math.floor(self.offset or 0), (self.offset or 0);
end

function ExtVendor_HybridScrollFrameScrollChild_OnLoad (self)
	self:GetParent().scrollChild = self;
end

function ExtVendor_HybridScrollFrame_ExpandButton (self, offset, height)
	self.largeButtonTop = round(offset);
	self.largeButtonHeight = round(height)
	ExtVendor_HybridScrollFrame_SetOffset(self, self.scrollBar:GetValue());
end

function ExtVendor_HybridScrollFrame_CollapseButton (self)
	self.largeButtonTop = nil;
	self.largeButtonHeight = nil;
end

function ExtVendor_HybridScrollFrame_SetOffset (self, offset)
...
function ExtVendor_HybridScrollFrame_CreateButtons (self, buttonTemplate, initialOffsetX, initialOffsetY, initialPoint, initialRelative, offsetX, offsetY, point, relativePoint)
